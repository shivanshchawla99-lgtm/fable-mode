...
| Metric | Default Mode | Fable Mode | Change | Notes |
| :--- | :---: | :---: | :---: | :--- |
| **Input Word Count** | 1,003 | 3,439 | +242% | Increase is due to the skill context |
| **Output Word Count** | 7,557 | 5,871 | -22% | |
| **Expected Price / Agent** | $0.155 | $0.131 | -15.5% | |

The Fable system prompt is large, so input tokens rose. But the output dropped by 22 percent because the agents stopped generating boilerplate. Since output tokens cost significantly more than input tokens, the net result was a 15.5 percent reduction in total API costs per agent.
